    ...
    if let Ok(content) = std::fs::read_to_string(&pkg_json_path) {
        if let Ok(data) = serde_json::from_str::<serde_json::Value>(&content) {
            if let Some(deps) = data.get("dependencies").and_then(|d| d.as_object()) {
                for (pkg, _) in deps {
                    let node_module_pkg = project_path
                        .join("node_modules")
                        .join(pkg)
                        .join("package.json");
                    if node_module_pkg.exists() {
                        if let Ok(mod_content) = std::fs::read_to_string(&node_module_pkg) {
                            if let Ok(mod_data) =
                                serde_json::from_str::<serde_json::Value>(&mod_content)
                            {
                                if let Some(scripts) =
                                    mod_data.get("scripts").and_then(|s| s.as_object())
                                {
                                        ...
                                        || scripts.contains_key("install")
                                    {
                                        risks.push(SupplyChainRisk {
                                            ...
                                            description:
                                                "Package executes scripts automatically on install"
                                                    .to_string(),
                                        });
                                    }
                                }
                                if mod_data.get("deprecated").is_some() {
                                    risks.push(SupplyChainRisk {
                                        ...
                                        description:
                                            "Package is marked as deprecated by its author"
                                                .to_string(),
                                    });
                                }
                            }
...
pub async fn scan() -> ScanResult {
    ...
    if n > 0 {
        result
            .issues
            .push(format!("{n} supply chain risk(s) detected"));
    }
